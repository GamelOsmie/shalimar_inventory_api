...
from sales.models import Retail
from sales.serializer import RetailSerializer
from units.models import Branch
...
class BranchSalesStats(APIView):
    ...
    def get(self, request):
        ...
        retail_query = Retail.objects.prefetch_related('spare_parts_sold', 'vehicles_sold' ).filter(branch=branch)
        ...
        # day's stats 
        daily_data = retail_query.prefetch_related('spare_parts_sold', 'vehicles_sold').filter(sold_at__year=current_year).filter(sold_at__week=current_week).filter(sold_at__gt=yesterday).values('vehicles_sold', 'spare_parts_sold')
        ...
        # week's stats 
        weekly_data = retail_query.prefetch_related('spare_parts_sold', 'vehicles_sold').filter(sold_at__year=current_year).filter(sold_at__week=current_week).values('vehicles_sold', 'spare_parts_sold')
        ...
        # month's stats 
        monthly_data = retail_query.prefetch_related('spare_parts_sold', 'vehicles_sold').filter(sold_at__year=current_year).filter(sold_at__month=current_month).values('vehicles_sold', 'spare_parts_sold')
        ...
        yearly_data = retail_query.prefetch_related('spare_parts_sold', 'vehicles_sold').filter(sold_at__year=current_year).values('vehicles_sold', 'spare_parts_sold')
        ...
        sales_for_the_year = retail_query.prefetch_related('spare_parts_sold', 'vehicles_sold').filter(sold_at__year=current_year)
...
class BranchSalesTrend(APIView):
    ...
    def get(self, request):
        ...
        all_time_sales = Retail.objects.filter(branch=branch)
